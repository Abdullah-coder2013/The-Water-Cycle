...
public class MiniGameRiver : MonoBehaviour
{
    ...
    public int sceneSegmentsToSpawn = 3; // Number of segments to spawn initially
    ...
    private IEnumerator StartFactsWithDelay()
    {
        ...
        Information.Instance.StartFactSequence(WaterCycleStage.River);
    }
    ...
    private void GenerateObstaclesForSceneSegment(GameObject segment)
    {
        ...
        {
            var pos1 = segment.transform.GetChild(0).position;
            var pos2 = segment.transform.GetChild(1).position;
            // Instantiate obstacles at random positions within the segment
            Vector3 position = new Vector3(Random.Range(pos1.x, pos2.x),Random.Range(pos1.y, pos2.y) ,0);
            ...
            obstacle.transform.localScale = new Vector3(0.5f, 0.5f, 0); // Set the scale of the obstacle
